...
def customer_signup(request):
    ...
    user = User(
        username=data['email'],  # Assuming you're using email as username
        email=data['email'],
        first_name=data['first_name'],
        last_name=data['last_name']
    )
    user.set_password(data['password'])  # Hashes the password before saving
    user.save()

    # Link to the Customer profile
    customer = Customer.objects.create(user=user, **{k: data[k] for k in ['phone_number', 'address'] if k in data})
