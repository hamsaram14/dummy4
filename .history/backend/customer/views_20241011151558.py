...
def customer_signup(request):
    ...
    # Original password from the request
    original_password = data['password']
    print("Original password:", original_password)  # Debug: Print the original password
    ...
    # Pass modified data with hashed password to the serializer
    serializer = CustomerSerializer(data=data)

    if serializer.is_valid():
        customer = serializer.save()
        print("Customer saved with hashed password:", customer.password_hash)  # Confirm password is saved hashed
        return Response(serializer.data, status=status.HTTP_201_CREATED)
...
def customer_login(request):
    ...
    try:
        ...
        print("User does not exist for email:", email)  # Debug log
        return Response({'error': 'Invalid email or password'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    print("Password verification result:", password_valid)  # Debug log
    ...
    if password_valid:
        ...
        print("Invalid password for user:", email)  # Debug log
        return Response({'error': 'Invalid email or password'}, status=status.HTTP_400_BAD_REQUEST)
