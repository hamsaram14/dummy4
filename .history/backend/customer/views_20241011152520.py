...
def customer_signup(request):
    ...
    try:
        ...
        user.set_password(data['password'])   # This properly hashes the password
        user.save()

        # Create the associated Customer profile
        Customer.objects.create(user=user)  # Assuming OneToOne relationship with User model

        return Response({"message": "Sign-up successful!"}, status=status.HTTP_201_CREATED)
    except Exception as e:
        print("Error during signup:", e)  # Debugging
        return Response({"error": "Server error during signup"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def customer_login(request):
    ...
    try:
        ...
        print("User with email not found")  # Debugging
        return Response({'error': 'Invalid email or password'}, status=status.HTTP_400_BAD_REQUEST)

    try:
        # Verify the password
        if user.check_password(password):  # This uses Django's built-in check_password method
            refresh = RefreshToken.for_user(user)
            return Response({
                'message': 'Login successful',
        ...
        print("Error during login:", e)  # Detailed error output
